# Troubleshooting — IT-Stack ELASTICSEARCH
...
4. Verify port is not already in use: ss -tlnp | grep 9200
...
```bash
# Test connectivity
psql -h lab-db1 -U elasticsearch_user -d elasticsearch_db -c '\conninfo'
...
1. Check resource usage: docker stats it-stack-elasticsearch
...
| Log | Path |
|-----|------|
| Application | docker compose logs elasticsearch |
